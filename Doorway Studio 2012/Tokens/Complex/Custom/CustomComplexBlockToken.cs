...
{
    public class CustomComplexBlockToken : MinMaxExtendableToken, IComplexToken, ITokensRegexCompatible, ITokenTemplateEditorCompatible
    {
        public override bool Bold { get { return true; } }

        public override Color Color { get { return Color.Red; } }

        public override string Name { get { return "[CustomBlock]"; } }

        public override string[] Tokens { get { return new string[] { "[CustomBlock:X:X]", "[/CustomBlock]" }; } }
        ...
        protected string[] RegexTokensStart { get { return new string[] { @"(\[|{)CustomBlock:[0-9]+:[0-9]+(\]|})" }; } }
        protected string[] RegexTokensEnd { get { return new string[] { @"(\[|{)/CustomBlock(\]|})" }; } }

        public string[] Token
        {
            get
            {
                string begin = "[CustomBlock:5:10] [CustomLink] ";
                string end = " [/CustomBlock:5:10]";
                return new string[]
                           {
                               begin + "[CustomKeyword]" + end,
                               begin + "[CustomKeyword:Big]" + end,
                               begin + "[CustomKeyword:All]" + end,
                               begin + "[CustomKeyword:Com]" + end,
                               begin + "[CustomKeyword:BigAllWords]" + end,
                               begin + "[CustomKeyword:BigBigWords]" + end
                           };
            }
        }
        ...
        public string Invoke(IPage Page, IContentContainer ContentContainer)
        {
            ...
            for (int i = 0; i < this.RegexTokens.Length; i++)
            {
                ...
                while (Regex.IsMatch(content, this.RegexTokens[i], RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.Singleline))
                {
                    ...
                    // Looking for last [customblock:X:X] before [/customblock]
            ...
            for (int i = 0; i < this.RegexLinkTokens.Length; i++)
            {
                ...
                for (int k = 0; k < matchCollection.Count; k++)
                {
                    while (content.Contains(matchCollection[k].Value))
                    {
                        content = content.ReplaceOne(matchCollection[k].Value, Page.Type == FileType.Custom ? Page.URL : "[KeywordLink]");
                    }
                }
            }
        ...
        public string[] RegexTokenTokens { get { return new string[] { @"(\[|{)CustomKeyword([a-zA-Z0-9 ]+)*(\]|})" }; } }

        public string[] TokenExtensions { get { return new string[] { "Big", "All", "Com", "BigAllWords", "BigBigWords" }; } }

        public string[] RegexLinkTokens { get { return new string[] { @"(\[|{)CustomLink(\]|})" }; } }
        ...
        protected string ReplaceBlockTokens(IPage Page, string Content, IContentContainer ContentContainer)
        {
            ...
            for (int i = 0; i < this.RegexLinkTokens.Length; i++)
            {
                ...
                {
                    Content = Regex.Replace(Content, this.RegexLinkTokens[i], Page.Type == FileType.Custom ? Page.URL : "[MenuLink]");
                }
            }
