...
{
    public class RandomWordComplexToken : MinMaxExtendableToken, IComplexToken, ITokensRegexCompatible, ITokenTemplateEditorCompatible
    {
        public override bool Bold { get { return true; } }

        public override Color Color { get { return Color.Red; } }

        public override string Name { get { return "[RandomWord]"; } }

        public override string[] Tokens { get { return new string[] { "[RandomWord(:X:X:X)]" }; } }

        public override string[] Extensions { get { return new string[] { "Big", "Minimum", "Maximum" }; } }

        public override int Priority { get { return 4; } }

        public string[] RegexTokens { get { return new string[] { @"(\[|{)RandomWord([a-zA-Z0-9 ]+)*(\]|})" }; } }

        public string[] Token { get { return new string[] { "[RandomWord]", "[RandomWord:Big]", "[RandomWord:2:5]" }; } }
                ...
                for (int k = 0; k < matchCollection.Count; k++)
                {
                    ...
                    while (content.Contains(matchCollection[k].Value))
                    {
                        ...
                        if (this.CheckMinMaxExtensions(matchCollection[k].Value))
                        {
                            ...
                            {
                                text += ContentContainer.Text.Select(StringSelectType.Word, big);
                            }
                        }
                        else
                        {
