...
{
    public class RandomPhraseComplexToken : MinMaxExtendableToken, IComplexToken, ITokensRegexCompatible, ITokenTemplateEditorCompatible
    {
        public override bool Bold { get { return true; } }

        public override Color Color { get { return Color.Red; } }

        public override string Name { get { return "[RandomPhrase]"; } }

        public override string[] Tokens { get { return new string[] { "[RandomPhrase(:X:X:X)]" }; } }

        public override string[] Extensions { get { return new string[] { "Big", "Com", "BigBigWords", "BigAllWords", "Minimum", "Maximum" }; } }

        public override int Priority { get { return 4; } }

        public string[] RegexTokens { get { return new string[] { @"(\[|{)RandomPhrase([a-zA-Z0-9 ]+)*(\]|})" }; } }

        public string[] Token { get { return new string[] { "[RandomPhrase]", "[RandomPhrase:Big]", "[RandomPhrase:Com]", "[RandomPhrase:BigBigWords]", "[RandomPhrase:BigAllWords]", "[RandomPhrase:5:10]" }; } }
                ...
                for (int k = 0; k < matchCollection.Count; k++)
                {
                    ...
                    while (content.Contains(matchCollection[k].Value))
                    {
                        ...
                        if (this.CheckMinMaxExtensions(matchCollection[k].Value))
                        {
                            ...
                            {
                                text += ContentContainer.Text.Select(StringSelectType.Phrase, big);
                            }
                        }
                        else
                        {
