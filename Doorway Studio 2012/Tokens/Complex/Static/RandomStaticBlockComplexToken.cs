...
namespace Umax.Plugins.Tokens.Complex.Static
{
    public class RandomCustomComplexToken : MinMaxExtendableToken, IComplexToken, ITokensRegexCompatible, ITokenTemplateEditorCompatible
    {
        public override bool Bold { get { return true; } }
        public override Color Color { get { return Color.Red; } }

        public override string Name { get { return "[RandomStaticBlock]"; } }

        public override string[] Tokens { get { return new string[] { "[RandomStaticBlock(:X:X)]" }; } }
        ...
        protected string[] RegexTokensStart { get { return new string[] { @"(\[|{)RandomStaticBlock:[0-9]+:[0-9]+(\]|})" }; } }
        protected string[] RegexTokensEnd { get { return new string[] { @"(\[|{)/RandomStaticBlock(\]|})" }; } }

        public string[] Token
        {
            get
            {
                string begin = "[RandomStaticBlock:5:10] [RandomStaticLink] ";
                string end = " [/RandomStaticBlock]";
                return new string[]
                           {
                               begin + "[RandomStaticKeyword]" + end,
                               begin + "[RandomStaticKeyword:Big]" + end,
                               begin + "[RandomStaticKeyword:All]" + end,
                               begin + "[RandomStaticKeyword:Com]" + end,
                               begin + "[RandomStaticKeyword:BigAllWords]" + end,
                               begin + "[RandomStaticKeyword:BigBigWords]" + end
                           };
            }
        }
        ...
        public string Invoke(IPage Page, IContentContainer ContentContainer)
        {
            ...
            for (int i = 0; i < this.RegexTokens.Length; i++)
            {
                ...
                while (Regex.IsMatch(content, this.RegexTokens[i], RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.Singleline))
                {
                    ...
                    // Looking for last [randomstaticblock:X:X] before [/randomstaticblock]
                    ...
                    while (content.Contains(matchCollection[k].Value))
                    {
                        content = content.ReplaceOne(matchCollection[k].Value,
                                                       pages.Count != 0
                                                           ...
                                                           : "[RandomKeywordLink" + (extensions.Count == 0 ? string.Empty : ":" + extensions.AsString(":")) + "]");
                    }
                }
            }
        ...
        protected virtual string ReplaceBlockTokens(IPage Page, string Content, IContentContainer ContentContainer)
        {
            ...
            for (int i = 0; i < this.RegexLinkTokens.Length; i++)
            {
                ...
                {
                    Content = Regex.Replace(Content, this.RegexLinkTokens[i], Page.Type == FileType.Static ? Page.URL : "[RandomKeywordLink]");
                }
            }
