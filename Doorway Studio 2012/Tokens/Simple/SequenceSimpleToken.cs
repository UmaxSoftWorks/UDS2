...
{
    public class SequenceSimpleToken : MinMaxExtendableToken, ISimpleToken, ITokensRegexCompatible, ITokenTemplateEditorCompatible
    {
        public override bool Bold { get { return true; } }

        public override Color Color { get { return Color.Black; } }

        public override string Name { get { return "[Sequence:X:X]"; } }

        public override int Priority { get { return 1; } }

        public override string[] Tokens { get { return new string[] { "[Sequence(:X:X)]" }; } }

        public override string[] Extensions { get { return new string[] { "Minimum", "Maximum" }; } }

        protected Random Random = new Random();

        public string[] RegexTokens { get { return new string[] { @"(\[|{)Sequence([a-zA-Z0-9 ]+)*(\]|})" }; } }

        public string[] Token { get { return new string[] { "[Sequence:1:10]" }; } }
            ...
            for (int i = 0; i < this.RegexTokens.Length; i++)
            {
                while (Regex.IsMatch(Content, this.RegexTokens[i], RegexOptions.Compiled | RegexOptions.IgnoreCase))
                {
                    ...
                    for (int k = 0; k < collection.Count; k++)
                    {
                        ...
                        Sequence sequence = new Sequence(minmax[0], minmax[1]);
                        ...
                        while (Content.Contains(collection[k].Value))
                        {
                            ...
                            Content = Content.Insert(matchPosition, sequence.Invoke().ToString());
                        }
                    }
                }
            }
