...
{
    public class ReplaceSimpleToken : MinMaxExtendableToken, ISimpleToken, ITokensRegexCompatible, ITokenTemplateEditorCompatible
    {
        ...
        public override string[] Tokens { get { return new string[] { "[Replace:X:X]", "[/Replace]" }; } }
        ...
        protected string[] RegexTokensStart { get { return new string[] { @"(\[|{)Replace:[^:]+:[^:]+(\]|})" }; } }
        protected string[] RegexTokensEnd { get { return new string[] { @"(\[|{)/Replace(\]|})" }; } }
        ...
        public string Invoke(string Content)
        {
            for (int i = 0; i < this.RegexTokens.Length; i++)
            {
                while (Regex.IsMatch(Content, this.RegexTokens[i], RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.Singleline))
                {
                    ...
                    {
                        tokenValue = tokenValue.Replace(extensions[0], extensions[1]);
                    }
