...
{
    public class EnDaySimpleToken : ExtendableToken, ISimpleToken, ITokensRegexCompatible, ITokenTemplateEditorCompatible
    {
        public override bool Bold { get { return true; } }

        public override Color Color { get { return Color.Black; } }

        public override string Name { get { return "[EnDay]"; } }

        public override int Priority { get { return 4; } }

        public override string[] Tokens { get { return new string[] { "[EnDay(:X:X)]" }; } }

        public override string[] Extensions { get { return new string[] { "Random", "Big" }; } }

        public string[] RegexTokens { get { return new string[] { @"(\[|{)EnDay([a-zA-Z0-9 ]+)*(\]|})" }; } }

        public string[] Token { get { return new string[] { "[EnDay]", "[EnDay:Big]", "[EnDay:Random]", "[EnDay:Random:Big]" }; } }

        public string Description(LanguageType Language)
        {
            return "Day in English";
        }
                ...
                for (int k = 0; k < matchCollection.Count; k++)
                {
                    ...
                    if (extensions.Contains("Random", true))
                    {
                        ...
                        while (Content.Contains(matchCollection[k].Value))
                        {
                            ...
                            string day = DaysEnum.English[this.Random.Next(DaysEnum.English.Length)].ToLower();
                    ...
                    {
                        string day = DaysEnum.English[(int)global::System.DateTime.Now.DayOfWeek].ToLower();
