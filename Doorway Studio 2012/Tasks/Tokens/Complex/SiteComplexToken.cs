...
{
    internal class SiteComplexToken : IComplexToken, ITokensRegexCompatible, ITokenTemplateEditorCompatible
    {
        public bool Bold { get { return true; } }

        public Color Color { get { return Color.Black; } }

        public string Name { get { return "[Site]"; } }

        public int Priority { get { return 1; } }

        public string[] Tokens { get { return new string[] { "[Site]" }; } }

        public string[] RegexTokens { get { return new string[] { @"(\[|{)Site(\]|})" }; } }

        public string[] Token { get { return new string[] { "[Site]" }; } }

        public string Description(LanguageType Language)
        {
            return "Site [Site] token";
        }
        ...
        public string Invoke(IPage Page, IContentContainer ContentContainer)
        {
            ...
            for (int i = 0; i < this.RegexTokens.Length; i++)
            {
                ...
                {
                    content = content.Replace(matchCollection[k].Value, ContentContainer.Extensions.Strings["SITE"]);
                }
            }
