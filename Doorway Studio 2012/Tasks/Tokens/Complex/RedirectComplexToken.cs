...
{
    internal class RedirectComplexToken : IComplexToken, ITokensRegexCompatible, ITokenTemplateEditorCompatible
    {
        ...
        public string[] Tokens { get { return new string[] { "[Redirect]" }; } }

        public string[] RegexTokens { get { return new string[] { @"(\[|{)Redirect(\]|})" }; } }

        public string[] Token { get { return new string[] { "[Redirect]" }; } }

        public string Description(LanguageType Language)
        {
            return "Site [Redirect] token";
        }
        ...
        public string Invoke(IPage Page, IContentContainer ContentContainer)
        {
            ...
            for (int i = 0; i < this.RegexTokens.Length; i++)
            {
                ...
                {
                    content = content.Replace(matchCollection[k].Value, ContentContainer.Extensions.Strings["REDIRECT"]);
                }
            }
