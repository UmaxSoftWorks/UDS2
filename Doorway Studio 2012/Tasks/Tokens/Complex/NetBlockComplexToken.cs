...
{
    internal class NetBlockComplexToken : MinMaxExtendableToken, IComplexToken, ITokensRegexCompatible, ITokenTemplateEditorCompatible
    {
        public override bool Bold { get { return true; } }

        public override Color Color { get { return Color.Red; } }

        public override string Name { get { return "[NetBlock:X:X]"; } }

        public override string[] Tokens { get { return new string[] { "[NetBlock:X:X]", "[/NetBlock]" }; } }

        public override string[] Extensions { get { return new string[] { "Minimum", "Maximum" }; } }

        public string[] Token { get { return new string[] { "[NetBlock:5:10]" }; } }

        public string Description(LanguageType Language)
        {
            return "Site [NetBlock] token";
        }
        ...
        protected string[] RegexTokensStart { get { return new string[] { @"(\[|{)NetBlock:[0-9]+:[0-9]+(\]|})" }; } }
        protected string[] RegexTokensEnd { get { return new string[] { @"(\[|{)/NetBlock(\]|})" }; } }
        ...
        public string Invoke(IPage Page, IContentContainer ContentContainer)
        {
            ...
            for (int i = 0; i < this.RegexTokens.Length; i++)
            {
                ...
                while (ContentContainer.Extensions.Lists["NETBLOCK"].Count != 0 && Regex.IsMatch(content, this.RegexTokens[i], RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.Singleline))
                {
                    ...
                    // Looking for last [netblock:X:X] before [/netblock]
                    ...
                    {
                        multiplyCount = ContentContainer.Extensions.Lists["NETBLOCK"].Count;
                    }
                    ...
                    {
                        if (ContentContainer.Extensions.Lists["NETBLOCK"].Count <= index)
                        {
                            index = 0;
                        }

                        blockContent.Append(this.ReplaceBlockTokens(ContentContainer.Extensions.Lists["NETBLOCK"][index], tokenValue, ContentContainer));
                        index++;
                    }
        ...
        public string[] RegexTokenTokens { get { return new string[] { @"(\[|{)NetKeyword([a-zA-Z0-9 ]+)*(\]|})" }; } }

        public string[] TokenExtensions { get { return new string[] { "Big", "All", "Com", "BigAllWords", "BigBigWords" }; } }

        public string[] RegexLinkTokens { get { return new string[] { @"(\[|{)NetLink(\]|})" }; } }

        protected string ReplaceBlockTokens(string Link, string Content, IContentContainer ContentContainer)
        {
            // Prepare
            List<string> parts = Link.Split(new char[] { '|' }, StringSplitOptions.RemoveEmptyEntries).ToList();
            string url = parts[0];
            List<string> tokens = parts.Cut(1);
            ...
            for (int i = 0; i < this.RegexLinkTokens.Length; i++)
            {
                ...
                {
                    Content = Regex.Replace(Content, this.RegexLinkTokens[i], url);
                }
            }
            ...
            for (int i = 0; i < this.RegexTokenTokens.Length; i++)
            {
                while (Regex.IsMatch(Content, this.RegexTokenTokens[i], RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.Singleline))
                {
                    ...
                    for (int k = 0; k < matchCollection.Count; k++)
                    {
                        ...
                        {
                            Content = Content.ReplaceOne(matchCollection[k].Value, this.ProcessExtensions(tokens, extensions));
                        }
                    }
                }
            }
