...
{
    internal class RSSComplexToken : IComplexToken, ITokensRegexCompatible, ITokenTemplateEditorCompatible
    {
        ...
        public string[] Tokens { get { return new string[] { "[Rss]" }; } }

        public string[] RegexTokens { get { return new string[] { @"(\[|{)Rss(\]|})" }; } }

        public string[] Token { get { return new string[] { "[Rss]" }; } }

        public string Description(LanguageType Language)
        {
            return "Site [Rss] token";
        }
        ...
        public string Invoke(IPage Page, IContentContainer ContentContainer)
        {
            ...
            for (int i = 0; i < this.RegexTokens.Length; i++)
            {
                ...
                {
                    content = content.Replace(matchCollection[k].Value, ContentContainer.Extensions.Strings["RSS"]);
                }
            }
