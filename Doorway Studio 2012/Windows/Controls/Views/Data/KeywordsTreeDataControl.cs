...
{
    public partial class KeywordsTreeDataControl : UserControl
    {
        public KeywordsTreeDataControl()
        {
            InitializeComponent();
        }
        ...
        {
            get { return (this.SelectedWorkSpace != -1) && (this.SelectedKeywords != -1); }
        }

        private void KeywordsTreeDataControlLoad(object sender, EventArgs e)
        {
            if (DesignMode)
            {
                return;
        ...
        public void InitializeEvents()
        {
            ...
            {
                Core.Core.Instanse.Data[i].Keywords.CountChanged += this.EventHandler;
            }
        }
        ...
        public void UpdateControl()
        {
            ...
            for (int i = 0; i < Core.Core.Instanse.Data.Count; i++)
            {
                TreeNode[] itemTreeNode = new TreeNode[Core.Core.Instanse.Data[i].Keywords.Count];
                for (int k = 0; k < Core.Core.Instanse.Data[i].Keywords.Count; k++)
                {
                    itemTreeNode[k] = new TreeNode(Core.Core.Instanse.Data[i].Keywords[k].Name, 1, 1);
                }
        ...
        protected void mainTreeViewAfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            if (mainTreeView.SelectedNode != null)
            {
                ...
                if (indexes.Count > 1)
                {
                    this.SelectedKeywords = indexes[1];
                }
            }

            if (this.KeywordsChanged != null)
            {
        ...
        protected void Delete(object sender, EventArgs e)
        {
            ...
            if (WinHelper.MessageBox("Do you really want to delete this Keywords?", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
            {
                return;
            }

            (Core.Core.Instanse.Data[this.SelectedWorkSpace].Keywords[this.SelectedKeywords] as KeyWord).Delete();
            Core.Core.Instanse.Data[this.SelectedWorkSpace].Keywords.RemoveAt(this.SelectedKeywords);
        ...
        public event SimpleEventHandler KeywordsChanged;
            ...
            new ItemAction()
                {
                    ...
                    Element = DataElementType.Keywords,
            ...
            new ItemAction()
                {
                    ...
                    Element = DataElementType.Keywords,
                    WorkSpace = Core.Core.Instanse.Data[this.SelectedWorkSpace],
                    Item = Core.Core.Instanse.Data[this.SelectedWorkSpace].Keywords[this.SelectedKeywords]
                }.Show(FindForm());
        }
